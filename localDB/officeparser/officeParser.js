...
const pdfjs         = require('officeparser/pdfjs-dist-build/pdf.js');
 ...
 * @property {boolean} [outputErrorToConsole] Flag to show all the logs to console in case of an error irrespective of
 *   your own handling. Default is false.
 * @property {string}  [newlineDelimiter]     The delimiter used for every new line in places that allow multiline text
 *   like word. Default is \n.
 * @property {boolean} [ignoreNotes]          Flag to ignore notes from parsing in files like powerpoint. Default is
 *   false. It includes notes in the parsed text by default.
 * @property {boolean} [putNotesAtLast]       Flag, if set to true, will collectively put all the parsed text from
            ...
            xmlContentArray.forEach(xmlContent => {
                ...
                responseText.push(
                    Array.from(xmlParagraphNodesList)
                      ...
                      // todo @ANKU @LOW - парсинг строчек через 'w:br'
                        // .map(paragraphNode => {
                ...
                if (!node.childNodes || node.childNodes.length == 0) {
                    if (node.parentNode.tagName.indexOf('text') == 0 && node.nodeValue) {
                        if (isNotesNode(node.parentNode) && (config.putNotesAtLast || config.ignoreNotes)) {
                          ...
                          const currentPrevSibling = node.previousSibling
                          // может быть 'text:soft-page-break' перед "text:line-break"
                          const parentParentPrev = node.parentNode.previousSibling && node.parentNode.previousSibling.nodeName === 'text:soft-page-break'
                            ? node.parentNode.previousSibling.previousSibling
                            : node.parentNode.parentNode.previousSibling

                          // может не быть если это prev.nodeName: "text:soft-page-break"
                          const prevLastChild = parentParentPrev && parentParentPrev.childNodes[parentParentPrev.childNodes.length - 1]
                          // console.log('ANKU , node', (
                          //     (currentPrevSibling && currentPrevSibling.nodeName === "text:line-break")
                          //     || (prevLastChild && prevLastChild.nodeName === "text:line-break")
                          //   ), node.nodeValue, node)
                          ...
                          if (
                            allowedTextTags.includes(node.parentNode.tagName)
                            && !isFirstRecursion
                          ) {
                            xmlTextArray.push(config.newlineDelimiter ?? "\n");
                          }
...
function parseTxt(file, callback, config) {
        ...
        .then(({ file, ext }) => {
            ...
            switch (ext) {
                ...
                case "pdf":
                    ...
                    break;
                case "txt":
                    parseTxt(file, internalCallback, internalConfig);
                    break;
 ...
 * @param {(x: string) => boolean} filterFn A function that receives the entry object and returns true if the file
 *   should be extracted.
...
if ((typeof process.argv[0] == 'string' && (process.argv[0].split('/').pop() == "node" || process.argv[0].split('/').pop() == "npx")) &&
    ...
    if (fileArg != undefined) {
         ...
         * @param {string[]} args List of string arguments that we need to parse to understand the config flag they
         *   represent.
         */
