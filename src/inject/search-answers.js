...
// function normalizeArrayAnswer(answer) {
//   return latinToViewCyrillic(answer)
// }

function mergeAnswersMap(dest = {}, newMap = {}) {
  return Object.keys(newMap).reduce((result, key) => {
    const questionKey = normalizeTextCompare(key, true)

    const newAnswers = newMap[key].map((answer) => normalizeTextCompare(answer, true))

    if (result[questionKey]) {
      // мержим уникальные
      result[questionKey] = [...new Set([
        ...result[questionKey],
        ...newAnswers,
      ])]
    } else {
      result[questionKey] = newAnswers
    }
    return result
  }, dest)
}

export async function searchAnswers2(certName) {
  log('ТЕМА:\n', certName)
  const certNameNormalized = normalizeTextCompare(
    certName
      .replace(/\s?-?\s?\d{4}$/gi, ''), // удаляем год
  ...
  const results = await Promise.all(SEARCH_ADAPTERS.map(async (adapter) => {
    ...
    log(adapterLogPrefix, adapter.isLocal ? 'Локальная база' : `Сайт - ${adapter.domainUrl}`)
    ...
    const topicResults = await Promise.all(topicSearchItems.map(async (item) => {
      ...
      const linkTitleHash = normalizeTopicId(linkTitle)
      if (linkTitleHash.indexOf(topicPartId) >= 0) {
        // тема подходит
        const topicAnswersMap = await adapter.findAnswersMap(content)
        logDebug(adapterLogPrefix, `Ответы на тему [${Object.keys(topicAnswersMap).length}]: ${linkTitle}\n`, topicAnswersMap)
        return topicAnswersMap
      } else {
        logDebug(adapterLogPrefix, 'Сравнение не короткое\n', linkTitleHash, linkTitleHash.length, '\n', topicPartId, topicPartId.length)
      }
      return {}
    }))

    return topicResults.reduce(mergeAnswersMap, {})
  }))

  const answersMapFull = results.reduce(mergeAnswersMap, {})
  log('СМЕРЖЕННЫЕ ОТВЕТЫ', Object.keys(answersMapFull).length, '\n', answersMapFull)
  return answersMapFull
}
...
//
//   log('ОТВЕТЫ - ', Object.keys(answersMap).length, '\n', answersMap)
//   return answersMap
//
